...
export default function MasterWinLossChart({ analysisData = [], winPropName, lossPropName, type, period = '24h' }) {
   ...
   analysisData.map(result => {
      dataSet[0].values.push({ time: new Date(result.refDate).getTime(), value: result[winPropName] });
      dataSet[1].values.push({ time: new Date(result.refDate).getTime(), value: result[lossPropName] });
   });
